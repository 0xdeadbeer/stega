...
def main(): 
    ...
    if ((y_range_width * x_range_width) <= len(data_bit_array)):
        print ("BIG PROBLEM WE CANNOT FIT YOUR DATA INTO THE IMAGE LOL")
        return 
        ...
        for y_index in range(y_pos, y_end): 
            for x_index in range(x_pos, x_end):
                ...
                try: 
                    ...
                    print (e)
                    print ("Exited at index -> " + str(index))
                    # alright we're done here 
                    raise Found 

    except Found: 
        new_location = location + ".hid.png"

        cv2.imwrite(new_location, image_data)
        print ("Image written to " + new_location) 
